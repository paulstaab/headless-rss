...
@app.get("/folders/", response_model=FolderGetOut)
def get_folders() -> FolderGetOut:
    db = database.get_session()
    folders = db.query(database.Folder).all()
    return FolderGetOut(folders=[schema.Folder.model_validate(folder) for folder in folders])
...
@app.post("/folders/", response_model=FolderPostOut)
def create_folder(input: FolderPostIn):
    ...
    return {"folders": get_folders().folders}
