//! Regression tests covering the composed API router and shared request helpers.
...
/// Builds application state for API tests with the provided in-memory database.
...
async fn updated_items_feed_selection_honors_last_modified_threshold() {
    ...
    let response = app(state(pool))
        .oneshot(
            Request::builder()
                .uri("/index.php/apps/news/api/v1-3/items/updated?lastModified=180&type=0&id=10")
                .body(Body::empty())
                .unwrap(),
        )
        .await
